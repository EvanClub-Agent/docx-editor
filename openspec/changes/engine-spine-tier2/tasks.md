> **STATUS (2026-06-05): the CLEAN HALF shipped in PR #710.** Steps 0-3 are done — geometry→core, `computeLayout`, the rAF `layoutScheduler`, and the shared `stripScrollFlag` (the safe part of the transaction loop). Both adapters delegate; verified + reviewed. The dispatch _orchestration_ was intentionally NOT unified (reactivity models diverge). REMAINING: step 4 (view lifecycle) and step 5 (load/save source-of-truth inversion, gated on #89) — tracked in the `engine-divergence-tier3` change alongside the Tier 3 divergence resolutions.
